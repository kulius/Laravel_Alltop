...
@section('content')
    ...
    if(isset(old("SystemName_srh")[0])){
        $aBody[] = array('flex' => 'auto', 'body' => $eZui->setBtnHref(array('ex' => 'add', 'route' => 'm07120_view', 'param' => array('add','?SystemName='. old('SystemName_srh')[0]))));
    } else {
    ...
    $aField[] = array('head' => __('需求時間'), 'name' => 'DemandTime', 'width' => '5%');
    ...
    foreach ($data as $skData => $svData) {
         ...
         $sDemandTime      = trim($svData['DemandTime']);
        ...
        $aBtn   = array();
        $aBtn[] = $eZui->setBtnHref(array('ex' => 'view', 'small' => true, 'route' => 'm07120_view', 'param' => array('view',$sID)));
        $aBtn[] = $eZui->setBtnHref(array('ex' => 'edit', 'small' => true, 'route' => 'm07120_view', 'param' => array('edit',$sID)));
        ...
        $aData[$skData]['DemandTime']      = $sDemandTime;
